(function () {
  ...
  var themes = [
    ...
    {id:'companies',number:'04',name:'公司与战略研究',short:'Companies',desc:'围绕产业链地位、资本开支、商业模式和估值的公司专题。',test:/spacex|stripe|韩国|korea|financial ai|银行|公司|equity|三星|海力士|台积电|美国ai半导体/i},
  ...
  function renderStats() {
    ...
    setText('stat-themes', themes.length);
  ...
  function renderTicker() {
    ...
    if (!el) return;
    el.innerHTML = allPages.slice(0, 7).map(function (p) {
      return '<a class="ticker-item" href="' + href(p) + '"><b>' + esc(shortTitle(p.title)) + '</b><span>' + esc(compactDate(p.date)) + '</span></a>';
    }).join('');
  }

  function renderFeatured() {
    var priority = ['hyperliquid-financial-market-infrastructure-20260718','us-ai-semiconductor-reshoring-memory-fabs-20260717','stripe-payment-ecosystem-2026'];
    var chosen = priority.map(findBySlug).filter(Boolean);
    allPages.forEach(function (p) { if (chosen.length < 3 && chosen.indexOf(p) < 0) chosen.push(p); });
  ...
  function renderThemes() {
    ...
    el.innerHTML = themes.map(function (t) {
      var count = allPages.filter(function (p) { return t.test.test(haystack(p)); }).length;
      t.test.lastIndex = 0;
  ...
  function renderArticles() {
    ...
    var filtered = allPages.filter(function (p) {
      ...
      var themeOk = !theme || theme.test.test(haystack(p));
      if (theme) theme.test.lastIndex = 0;
      return themeOk && (!query || haystack(p).indexOf(query) >= 0);
    });
    if (!filtered.length) {
  ...
  function articleCard(p) {
    ...
    return '<article class="article"><div class="article__meta"><span>' + esc(p.date || '') + '</span><span class="article__tag">' + esc((theme && theme.short) || p.tag || 'Research') + '</span></div><h3>' + esc(p.title || '未命名专题') + '</h3><p>' + esc(p.deck || '') + '</p><div class="article__footer"><span>KERWIN RESEARCH</span><span class="article__arrow">↗</span></div><a href="' + href(p) + '" aria-label="阅读 ' + esc(p.title || '') + '"></a></article>';
  }

  function href(p) { return p.source === 'codex' ? '/' + p.slug + '.html' : '/' + p.slug + '/'; }
  function findBySlug(slug) { return allPages.find(function (p) { return p.slug === slug; }); }
  function haystack(p) { return [p.title,p.deck,p.tag,p.slug].join(' ').toLowerCase(); }
