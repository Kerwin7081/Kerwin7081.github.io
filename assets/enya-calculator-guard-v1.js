...
const tools=document.createElement('div');tools.className='enya-model-tools';tools.dataset.enyaComponent='model-toolbar';tools.innerHTML='<button type="button" data-reset>恢复原始假设</button><span class="enya-reader__note">交互测算仅用于建立量级感，不代表公司实际合同或财务预测。</span><span class="enya-model-message" role="status" aria-live="polite"></span>';host.before(tools);
const status=tools.querySelector('.enya-model-message');const reset=tools.querySelector('[data-reset]');
fields.forEach((field,index)=>{const label=field.closest('.field')?.querySelector('label');if(label&&!field.getAttribute('aria-label'))field.setAttribute('aria-label',label.textContent.trim());field.dataset.enyaDefault=defaults[index];field.dataset.enyaModelInput=''});
